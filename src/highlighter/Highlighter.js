...
export default class Highlighter {
    ...
    applyStyles = (annotation, spans) => {
        const extraClasses = this.formatter ? this.formatter(annotation).className : '';
        ...
        spans.forEach(span => {
            ...
            if (comment) {
                ...
                span.addEventListener('mouseout', this.onBlur)
            }
        });
    }

    onBlur = (event) => {
        const marks = event.target.querySelectorAll(".mark-bubble");
    ...
    onHover = (event) => {
        ...
        //Remove old ones (just in case)
        const marks = target.querySelectorAll(".mark-bubble");
        marks.forEach((mark) => mark.remove());

        //Get the text for the bubble
        const text = target.getAttribute("data-mark");
        ...
        const formattedWidth = Math.ceil(width + 22) + "px";

        //Add the bubble
        const bubble = document.createElement("div");
        bubble.className = "mark-bubble";

        const bubbleContent = document.createElement("div");
        bubble.appendChild(bubbleContent);

        if (width < 350) {
            bubble.style.width = formattedWidth;
        }
